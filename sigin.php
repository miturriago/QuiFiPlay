<?php
  ...
  if (!empty($_POST['email']) && !empty($_POST['pass'])) {
    $records = $conn->prepare('SELECT nombre, email, pass FROM estudiante WHERE email = :email and pass= :pass');
    ...
    if ($records->execute()) {
		...
		$_SESSION['correo'] = $results['email'];

		$nombre=$_SESSION['user_id'];
		$email=$_SESSION['correo'];
		if (!empty($nombre)) {
			header("Location: index2.php?nombre=$nombre&email=$email");
		}
...
<body>
	...
	<div class="wrap">
		...
		<p> ¿No tiene una cuenta? <a href="register.php"> Click aquí </a></p>
